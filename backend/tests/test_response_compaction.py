...
def test_generate_compacts_debug_payload_when_disabled() -> None:
    settings = Settings(
    ...
    async def fake_sight_gather(
        ...
        trace.append(ToolCallRecord(tool_name="maps_text_search", summary="ok"))
    ...
    async def fake_weather_gather(
        ...
        return WeatherSummary(daily_forecasts=[])
    ...
    assert result.planning_context.destination == "上海"
    assert result.planning_context.attractions == []
    assert result.planning_context.restaurants == []
    assert result.planning_context.hotels == []
