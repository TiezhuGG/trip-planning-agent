...
def test_generate_runs_hotel_and_weather_in_parallel() -> None:
    ...
    result = asyncio.run(
        asyncio.wait_for(
            coordinator.generate(request, generated_at=datetime.now(UTC)),
            timeout=1.0,
        )
    )
...
def test_generate_exposes_fallback_flag_from_agents() -> None:
    ...
    async def fake_seed_gather(
        ...
        trace = AgentExecution(agent_name="planner_seed_agent", success=True, used_llm=False)
        return draft, trace, False, True, ["seed fallback"]
